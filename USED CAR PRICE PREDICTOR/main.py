...
import numpy as np
import joblib
# input dictionary for categorical label
# brand dict
brand_dic = {'Audi': 0,
 'BMW': 1,
 'Mercedes-Benz': 2,
...
brand_list = ['Volkswagen',
 ...
 'Audi',
 'Mitsubishi']
body_list = ['crossover', 'hatch', 'other', 'sedan', 'wagon', 'van']
engine_type_list = ['Diesel', 'Gas', 'Other', 'Petrol']
registration_list = ['no', 'yes']

# setting custom tab
st.set_page_config(page_title='Used Car price Prediction by Mwarano')
...
# loading both models
with st.spinner('Web app is loading!! '):
    model_forest = model_loader("rf1_base_rf.pkl")

# # writing header
# st.title('# Used Car Price Prediction™')
st.markdown("<h2 style='text-align: center;'> Used Car Price Predictor by Mwarano Martin </h2>", unsafe_allow_html=True)

col1, col2 = st.columns(2) #creating the 2 layouts

# start taking inputs
# 1. taking mileage info integer
mileage = col1.number_input(label='Enter the mileage of the car,', help='how much the car has  driven?', step = 1,min_value=0)

# 3. year integer
year =  col1.slider('Enter the year when the car was manufactured, e.g: 2015', 1980,2020,2005,help='The year when the car is manufactured.')
...
# 6. engine type
engine_type = col1.selectbox(label='Enter the fuel type', options=engine_type_list, help='In which gas the car is run?')
...
if brand_inp == 'Audi':
    ...
    model = model_dic[model_inp]  # doing numerical encoding which refers to the process of converting categorical or text data into numerical form so that it can be used in machine learning algorithms

elif brand_inp == 'Mitsubishi':
    model_inp = col2.selectbox('Enter the model for Mistubishi', options=find_model('Mitsubishi'))
...
regis = col2.selectbox(label='The car has registration?', options=registration_list, help='the car has been registered or not?')
